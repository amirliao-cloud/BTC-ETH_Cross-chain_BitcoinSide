...
from Scripts.Script import Script, p2pkh_script
...
class Tx:
    ...
    # Transaction fees are calculated based on the size of the
    # transaction in kilobytes, not the value of the transaction in bitcoin.
    def fee(self):
    ...
    def sign_input_atomic_swap_true_branch(self, secret, input_index, private_key):
        '''Signs the input using the private key'''
        # get the signature hash (z)
        z = self.sig_hash(input_index)
        # get der signature of z from private key
        ...
        sec = private_key.point.sec()
        # initialize a new script with [sig, sec] as the cmds
        script_sig = Script([sig, sec, secret, 0x51]) # < sig, secret, op_true>
    ...
    def sign_input_atomic_swap_false_branch(self, secret, input_index, private_key):
        '''Signs the input using the private key'''
        # get the signature hash (z)
        z = self.sig_hash(input_index)
        # get der signature of z from private key
...
class TxIn:
    def __init__(self, prev_tx, prev_index, script_sig=None, sequence=0xffffffff-1):
...
class TxOut:
    ...
    def __init__(self, amount, script_pubkey):
        ...
        self.script_pubkey = script_pubkey #locking script, specify the conditions need to be satisfied to unlock
...
class OutputInfo:
    ...
    def select_outputs_greedy(unspent, min_value):
        ...
        key_funcs = lambda utxo: utxo.value
        if greaters:
        ...
        lessers.sort(key = key_funcs, reverse = True)
        result = []
        accum = 0
        for utxo in lessers:
            result.append(utxo)
    ...
    # Once the UTXO is selected, the wallet then produces unlocking scripts containing
    # signatures for each of the UTXO, thereby making them spendable by satisfying their locking
    # script conditions.
...
if __name__ == '__main__':
    ...
    raw_tx = 'dac93c96fc1fa1479d1b5efe2debd9980bb89a897d8fce2b07479c49ae731aee'
    stream = BytesIO(bytes.fromhex(raw_tx))
    transaction = Tx.parse(stream, False)
    ...
    print(OutputInfo.select_outputs_greedy(unspent, target))
